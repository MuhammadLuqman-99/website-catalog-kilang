          ...
          images(first: 5) {
            edges {
              node {
                ...
                width
                height
                transformedSrc(maxWidth: 1200, maxHeight: 1200, preferredContentType: PNG)
              }
            }
          }
          variants(first: 10) {
...
export const getProductImageUrl = (product, size = 'medium') => {
  ...
  // Always prefer transformedSrc (PNG) over original url
  if (imageNode.transformedSrc) {
    console.log('Using transformedSrc:', imageNode.transformedSrc);
    return imageNode.transformedSrc;
  }

  let imageUrl = imageNode.url;
  console.log('Original imageUrl:', imageUrl);

  // Force PNG format for ALL Shopify images
  if (imageUrl.includes('cdn.shopify.com')) {
    ...
    // Remove ALL existing parameters and formats
    imageUrl = imageUrl.split('?')[0]; // Remove query params
    imageUrl = imageUrl.replace(/_\d+x\d+/g, ''); // Remove existing size suffix
    imageUrl = imageUrl.replace(/\.webp$/, ''); // Remove .webp extension
    imageUrl = imageUrl.replace(/\.jpg$/, ''); // Remove .jpg extension
    imageUrl = imageUrl.replace(/\.jpeg$/, ''); // Remove .jpeg extension

    // Force PNG format with specific parameters
    imageUrl = `${imageUrl}.png?width=${targetSize.split('x')[0]}&height=${targetSize.split('x')[1]}&format=png&quality=90`;
    console.log('Final PNG URL:', imageUrl);
  }
